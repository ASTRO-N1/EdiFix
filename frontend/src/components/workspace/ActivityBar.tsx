...
export default function ActivityBar() {
  ...
  return (
    <div
      ...
      // Set hover true only if we aren't already permanently expanded
      onMouseEnter={() => {
        if (!isAlwaysExpanded) setHovered(true)
      }}
      // Clear hover AND close the dropdown if the mouse leaves the sidebar area
      onMouseLeave={() => {
        if (!isAlwaysExpanded) {
          setHovered(false)
          setOtherOpen(false)
