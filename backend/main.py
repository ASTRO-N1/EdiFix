...
# ── WhatsApp Bot: Download fixed EDI file ─────────────────────────────────────
@app.get("/api/download/{token}")
async def download_fixed_edi(token: str):
    """
    Serves the fixed EDI file for download.
    The token is a hash of the user's phone number, generated by the bot.
    """
    from fastapi.responses import Response as FastAPIResponse
    import httpx as _httpx

    SUPABASE_URL_LOCAL = os.getenv("SUPABASE_URL")
    SUPABASE_KEY_LOCAL = os.getenv("SUPABASE_SERVICE_ROLE_KEY")
    headers = {
        "apikey": SUPABASE_KEY_LOCAL,
        "Authorization": f"Bearer {SUPABASE_KEY_LOCAL}",
    }

    # Find session by token (we store token as sha256[:16] of phone_number)
    # We scan all sessions and match — acceptable since session count is tiny
    async with _httpx.AsyncClient() as client:
        resp = await client.get(
            f"{SUPABASE_URL_LOCAL}/rest/v1/whatsapp_sessions",
            headers=headers,
            params={"select": "phone_number,current_tree,edi_output"},
        )
    ...
    for s in sessions:
        import hashlib
        phone_token = hashlib.sha256(s["phone_number"].encode()).hexdigest()[:16]
    ...
    # Use pre-generated edi_output if available, otherwise generate now
    edi_string = matched.get("edi_output")
    if not edi_string:
        try:
            tree = json.loads(matched["current_tree"])
            from core_parser.edi_generator import EDIGenerator as _EDIGenerator
            generator = _EDIGenerator(tree)
    ...
    return FastAPIResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=\"fixed_claim.edi\""},
    )
