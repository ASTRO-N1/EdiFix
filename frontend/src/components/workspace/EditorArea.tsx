    ...
    function RawEDIContent() {
      ...
      useEffect(() => {
        ...
        async function regenerate() {
          ...
          // If we have a parsed tree, generate fresh EDI from it (reflects form edits)
          if (parseResult) {
            try {
              ...
              if (!cancelled) {
                ...
                // Fallback: try reading from the original file
                await fallbackToFile()
              }
            }
          } else {
            // No parse result — fall back to reading the raw uploaded file
            await fallbackToFile()
          }
      ...
      const handleSave = async (e: React.MouseEvent) => {
        ...
        try {
          // Strip the newlines we added for readability
